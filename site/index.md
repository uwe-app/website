...
Use a combination of Markdown and your preferred web technology for unlimited expression; we like simplicity so lean towards pure HTML, CSS and Javascript but also support Tailwind, PostCSS and SASS via a powerful plugin system.
...
Interested? Head over to [install]({{ link "/install/" }}) then see [usage]({{ link "/usage/" }}) to get started and read the [docs]({{ link "/docs/" }}) for more information.
